...
def synthesize_in_chunks(model, text, language, gpt_cond_latent, speaker_embedding, max_chars=250):
    ...
    for i, chunk in enumerate(chunks):
        print(f"Chunk {i+1}: {chunk}",flush=True)
        sys.stdout.flush()
    audios = []
    ...
    for i, chunk in enumerate(chunks):
        ...
        print(f"Chunk {i+1}/{len(chunks)} synthesized in {elapsed:.2f}s",flush=True)
        sys.stdout.flush()
        audios.append(torch.tensor(out["wav"]))
    print(f"Total synthesis time: {time.time()-start_total:.2f}s",flush=True)
    sys.stdout.flush()
